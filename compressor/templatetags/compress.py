...
COMPASS = 'compressor.compass.CompassWrapper'
...
class CompassNode(CompressorNode):
    
    def __init__(self, nodelist, kind=None, mode=OUTPUT_FILE):
        self.wrapper = get_class(COMPASS, exception=ImproperlyConfigured)
        super(CompassNode, self).__init__(nodelist, 'css')

    def render(self, context, forced=False):
        self.wrapper().compile()
        return super(CompassNode, self).render(context, forced=False)
    
@register.tag
def comprass(parser, token):
    """
    Assuming a compass enviroment, compile project with compass and pass behaviour to django_compressor
    
    Syntax::

        {% comprass %}
            <linked css generated by compass>
            <linked/inline another css>
        {% endcomprass %}
    ...
    /compass/screen.sass
             print.sass
             ie.sass
    /static/css/ # Container to compass compile
    config.rb    # Compass config file, musted
        ...
        {% comprass %}
            ...
            <[endif]-->
            <link rel="stylesheet" href="/static/css/nocompass.css" type="text/css" />
        {% endcomprass %}
    """
    nodelist = parser.parse(('endcomprass',))
    parser.delete_first_token()

    return CompassNode(nodelist)
